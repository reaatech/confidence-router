...
import { EmbeddingSimilarityClassifier, KeywordClassifier, RouterFactory } from '../src/index.js';
...
describe('LLMClassifier Integration', () => {
  ...
  it('integrates with ConfidenceRouter for full LLM pipeline', async () => {
    ...
    const llmClassifier = new (
      await import('@reaatech/confidence-router-classifiers')
    ).LLMClassifier({
      provider: 'openai',
      apiKey: 'test',
